...
extern uint32_t
(**ppfcvImageSegmentationRegionGrow) 
    (const uint8_t* __restrict src, 
    uint32_t srcWidth, 
    uint32_t srcHeight, 
    uint32_t srcStride, 
...
extern int
(**ppfcvGeomHomographyEvaluatef32_v2)
( const fcvCorrespondences* __restrict corrs,
  float* __restrict                    homography,
  float                                maxsqerr,
  uint16_t* __restrict                 inliers,
  float*__restrict                     errinliers,
  int32_t*                             numinliers );
...
extern int
(**ppfcvPyramidCreatef32_v2)
( const float32_t* __restrict base, 
  uint32_t                    baseWidth,
  uint32_t                    baseHeight,
  uint32_t                    baseStride,
  uint32_t                    numLevels,
...
extern int
(**ppfcvPyramidAllocate_v2)
(
    fcvPyramidLevel_v2* pyr,
    unsigned int baseWidth,
    unsigned int baseHeight,
	unsigned int baseStride,
    unsigned int bytesPerPixel,
    unsigned int numLevels,
    int allocateBase
);
...
extern void 
(**ppfcv3ChannelTransformAffineClippedBCu8)
( const uint8_t *__restrict      src,
  uint32_t                       srcWidth,
  uint32_t                       srcHeight,
  uint32_t                       srcStride,
...
extern void
(**ppfcvImageDetectEdgePixelsu8)
( const int16_t* __restrict  gxgy,
...
extern void
(**ppfcvElementMultiplyf32)
( 
  const float32_t * __restrict src1,   
  uint32_t                     width,
  uint32_t                     height,
  uint32_t                     src1Stride,
  const float32_t * __restrict src2, 
...
extern void
(**ppfcvMatrixMultiplys8s32)
( 
  const int8_t * __restrict  src1, 
  uint32_t                   src1Width,			
  uint32_t                   src1Height,
  uint32_t                   src1Stride,
...
extern int
(**ppfcvClusterEuclideanu8)
(   const uint8_t* __restrict    points,
    int32_t                      numPoints,  // actually not used but helpful
    int32_t                      dim,
    int32_t                      pointStride,
    int32_t                      numPixels,
    int32_t                      numClusters,
...
inline int
	fcvGeomHomographyEvaluatef32_v2( const fcvCorrespondences* __restrict corrs,
                                     ...
                                     int32_t*                             numinliers )
{
   return (**ppfcvGeomHomographyEvaluatef32_v2)( corrs, homography, maxsqerr, inliers, 
	                                      errinliers, numinliers );
}
...
{
   srcStride = (srcStride==0 ? srcWidth : srcStride);
   dstIntgrlStride = (dstIntgrlStride==0 ? srcWidth*4 : dstIntgrlStride);
   dstIntgrlSqrdStride = (dstIntgrlSqrdStride==0 ? srcWidth*8 : dstIntgrlSqrdStride);
#ifndef FASTCV_DISABLE_API_ENFORCEMENT
   fcvAssert( ((int)src & 0xF) == 0 );     // 128-bit alignment
   fcvAssert( (srcStride & 0x7) == 0 );    // multiple of 8
   fcvAssert( (dstIntgrlStride & 0x7) == 0 );    // multiple of 8
   fcvAssert( (dstIntgrlSqrdStride & 0x7) == 0 );    // multiple of 8
   fcvAssert( (srcStride >= srcWidth) );   // Stride is at least as much as Width
   fcvAssert( (dstIntgrlStride >= srcWidth*4) );     //dstStride is at least as much as patch width
   fcvAssert( (dstIntgrlSqrdStride >= srcWidth*8) );     //dstStride is at least as much as patch width
#endif
   (**ppfcvIntegrateImageu8u64)( src, srcWidth, srcHeight, srcStride, 
	                             dstIntgrl, dstIntgrlSqrd, dstIntgrlStride, dstIntgrlSqrdStride );
}
                             ...
                             int32_t                     nPyramidLevels)
{
   ...
   (**ppfcvTrackLKOpticalFlowu8_v2)( src1, src2, width, height, stride,
                                   src1Pyr, src2Pyr,
                                   featureXY, featureXY_out,
...
inline int
fcvPyramidCreatef32_v2( const float32_t* __restrict base, 
                        uint32_t                    baseWidth, 
                        uint32_t                    baseHeight, 
                        uint32_t                    baseStride,
                        uint32_t                    numLevels, 
                        fcvPyramidLevel_v2*         pyramid )
{
   baseStride = (baseStride==0 ? baseWidth*4 : baseStride);
#ifndef FASTCV_DISABLE_API_ENFORCEMENT
   fcvAssert( ((int)base & 0xF) == 0 );           // 128-bit alignment
   #ifndef NDEBUG
   int div = (1 << (numLevels - 1)) - 1;
   #endif
   fcvAssert( ( (baseStride & div) == 0 ) & ( (baseHeight & div) == 0 ) );
#endif

   return (**ppfcvPyramidCreatef32_v2)( base, baseWidth, baseHeight, baseStride, numLevels, pyramid );
}
...
inline int
fcvPyramidCreateu8_v2( const uint8_t * __restrict base, 
                       uint32_t                   baseWidth, 
                       uint32_t                   baseHeight, 
                       uint32_t                   baseStride, 
                       uint32_t                   numLevels, 
                       fcvPyramidLevel_v2*        pyramid )
{
   ...
   fcvAssert( ( (baseStride & div) == 0 ) & ( (baseHeight & div) == 0 ) );
#endif
  return (**ppfcvPyramidCreateu8_v2)( base, baseWidth, baseHeight,baseStride, numLevels, pyramid );
                       ...
                       int32_t             allocateBase )
{
    ...
    return (**ppfcvPyramidAllocate_v2)(pyr,baseWidth,baseHeight,baseStride,bytesPerPixel,numLevels,allocateBase);
                     ...
                     uint32_t            startLevel )
{
    ...
    (**ppfcvPyramidDelete_v2)(pyr,numLevels,startLevel);
}
...
fcvMseru8_v2 ( void *mserHandle,
                ...
                uint16_t* __restrict pointsArray)
{
   return (**ppfcvMseru8_v2)(mserHandle, srcPtr, srcWidth, srcHeight, srcStride,
                     maxContours, numContours, recArray, numPointsInContour,
                     pointsArraySize, pointsArray);
}
...
fcvMserExtu8_v2 ( void *mserHandle,
                ...
                uint32_t* __restrict contourNodeCounter)
{
   return (**ppfcvMserExtu8_v2)(mserHandle, srcPtr, srcWidth, srcHeight, srcStride,
                                 maxContours, numContours, recArray, numPointsInContour,
                                 pointsArraySize, pointsArray, contourVariation, contourPolarity,
                                 contourNodeId, contourNodeCounter);
}
...
fcvMserNN8u8 ( void *mserHandle,
                ...
                uint16_t* __restrict pointsArray)
{
   return (**ppfcvMserNN8u8)(mserHandle, srcPtr, srcWidth, srcHeight, srcStride,
                     maxContours, numContours, recArray, numPointsInContour,
                     pointsArraySize, pointsArray);
}
...
fcvMserExtNN8u8 ( void *mserHandle,
                ...
                uint32_t* __restrict contourNodeCounter)
{
   return (**ppfcvMserExtNN8u8)(mserHandle, srcPtr, srcWidth, srcHeight, srcStride,
                                 maxContours, numContours, recArray, numPointsInContour,
                                 pointsArraySize, pointsArray, contourVariation, contourPolarity,
                                 contourNodeId, contourNodeCounter);
}

inline int fcvMserNN8Init(const uint32_t width,
                 const uint32_t height,
                 uint32_t delta,
                 uint32_t minArea ,
                 uint32_t maxArea ,
                 ...
                 void ** mserHandle )
{
    return (*ppfcvMserNN8Init)(width,height,delta,minArea,maxArea,maxVariation, minDiversity, mserHandle);
}
...
fcvImageSegmentationRegionGrow( const uint8_t* __restrict src, 
                                ...
                                uint8_t* __restrict data )
{
   srcStride = (srcStride==0 ? srcWidth*numChannel : srcStride);
   segLabelStride = (segLabelStride==0 ? srcWidth*4 : segLabelStride);

#ifndef FASTCV_DISABLE_API_ENFORCEMENT
   fcvAssert( (srcStride & 0x7) == 0 );            // multiple of 8
   fcvAssert( (segLabelStride & 0x7) == 0 );       // multiple of 8
#endif   

   return (**ppfcvImageSegmentationRegionGrow)( src, srcWidth, srcHeight, srcStride, numChannel, thresholdSplit, thresholdMerge,
                         segLabel, segLabelStride,data );
...
inline void
    ...
    uint8_t maxMod[3])
{
    return (**ppfcvConfigBGCodeBookModel) ( cbmodel, cbBound, minMod, maxMod );
}
                       ...
                       uint8_t               binary)
{
    ...
    (**ppfcvImageMomentsu8)( src, srcWidth, srcHeight, srcStride, moments, binary );
}
                       ...
                       uint8_t               binary)
{
    ...
    fcvAssert( ((int)src & 0xF) == 0 );          // 128-bit alignment
    fcvAssert( (srcStride & 0x7) == 0 );         // multiple of 8
    fcvAssert( (srcStride >= srcWidth*4) );        // at least as much as width
#endif

    (**ppfcvImageMomentss32)( src, srcWidth, srcHeight, srcStride, moments, binary );
}
                       ...
                       uint8_t                      binary)
{
    ...
    (**ppfcvImageMomentsf32)( src, srcWidth, srcHeight, srcStride, moments, binary );
}
                       ...
                       float32_t *__restrict       x)
{
    ...
    fcvAssert( ((int)x & 0xF) == 0 );            // 128-bit alignment
#endif
    return (**ppfcvSolveLDLf32)( A, b, diag, N, x );
}
                  ...
                  uint32_t                     N)
{
   ...
   fcvAssert( ((int)b & 0xF) == 0 );     // 128-bit alignment
#endif
    return (**ppfcvDotProductf32)( a, b, N );
}
                                      ...
                                      uint32_t *__restrict          dstBorder)
{
    ...
    fcvAssert( ((int)affineMatrix & 0xF) == 0 );       // 128-bit alignment
    fcvAssert( ((int)dstBorder & 0xF) == 0 );          // 128-bit alignment
    fcvAssert( (( srcStride == 0 ) && (srcWidth & 0x7) == 0 ) || ((srcStride & 0x7) == 0 && (srcStride>3*srcWidth) == 0) );         // multiple of 8
    fcvAssert( (( dstStride == 0 ) && (dstWidth & 0x7) == 0 ) || ((dstStride & 0x7) == 0 && (dstStride>3*dstWidth) == 0) );         // multiple of 8
#endif
    (**ppfcv3ChannelTransformAffineClippedBCu8)(src,srcWidth,srcHeight,srcStride,affineMatrix,dst,dstWidth,dstHeight,dstStride,dstBorder);
}
                          ...
                          fcvThreshType               thresholdType)
{
   ...
   fcvAssert( (dstStride >= srcWidth) );   // Stride is at least as much as Width
#endif
    (**ppfcvFilterThresholdOtsuu8)(src,srcWidth,srcHeight,srcStride,dst,dstStride,thresholdType);
}
                            ...
                            uint32_t* __restrict       coordEdgePixels  )
{
    ...
    fcvAssert( ((gradStride & 0x7) == 0) );         // multiple of 8
#endif
    (**ppfcvImageDetectEdgePixelsu8)( gxgy, mag, gradStride, topLeftX, topLeftY, width, height, gridSize,
                                            threshold, nEdgePixelsMax, nEdgePixels, coordEdgePixels);
}
...
{
   srcStride = (srcStride==0 ? srcWidth : srcStride);
   dstStride = (dstStride==0 ? srcHeight : dstStride);
#ifndef FASTCV_DISABLE_API_ENFORCEMENT
    fcvAssert( ((int)src & 0xF) == 0 );           // 128-bit alignment
    fcvAssert( ((int)dst & 0xF) == 0 );           // 128-bit alignment
    fcvAssert( (srcStride & 0x7) == 0 );          // multiple of 8
    ...
    (**ppfcvTransposeu8) (src, srcWidth, srcHeight, srcStride, dst, dstStride);
}
...
fcvTransposeu16( const uint16_t * __restrict  src, 
				 ...
				 uint32_t                     dstStride )
{
   srcStride = (srcStride==0 ? srcWidth*2 : srcStride);
   dstStride = (dstStride==0 ? srcHeight*2 : dstStride);
#ifndef FASTCV_DISABLE_API_ENFORCEMENT
    fcvAssert( ((int)src & 0xF) == 0 );           // 128-bit alignment
    fcvAssert( ((int)dst & 0xF) == 0 );           // 128-bit alignment
    fcvAssert( (srcStride & 0x7) == 0 );          // multiple of 8
    ...
    (**ppfcvTransposeu16) (src, srcWidth, srcHeight, srcStride, dst, dstStride);
}
...
{
   srcStride = (srcStride==0 ? srcWidth*4 : srcStride);
   dstStride = (dstStride==0 ? srcHeight*4 : dstStride);
#ifndef FASTCV_DISABLE_API_ENFORCEMENT
    fcvAssert( ((int)src & 0xF) == 0 );           // 128-bit alignment
    fcvAssert( ((int)dst & 0xF) == 0 );           // 128-bit alignment
    fcvAssert( (srcStride & 0x7) == 0 );          // multiple of 8
    ...
    (**ppfcvTransposef32) (src, srcWidth, srcHeight, srcStride, dst, dstStride);
}
...
inline void
fcvFlipu8( const uint8_t * __restrict  src, 
           uint32_t                    srcWidth,
           uint32_t                    srcHeight,
           uint32_t                    srcStride,
           uint8_t * __restrict        dst,
           ...
           fcvFlipDir			       dir )
{
   ...
   dstStride = (dstStride==0 ? srcWidth : dstStride);
    ...
    (**ppfcvFlipu8) (src, srcWidth, srcHeight, srcStride, dst, dstStride, dir);
}
...
inline void
fcvFlipu16( const uint16_t * __restrict  src, 
            uint32_t                     srcWidth,
            uint32_t                     srcHeight,
            uint32_t                     srcStride,
            uint16_t * __restrict        dst,
            ...
            fcvFlipDir			         dir)
{
   ...
   dstStride = (dstStride==0 ? srcWidth*2 : dstStride);
    ...
    (**ppfcvFlipu16) (src, srcWidth, srcHeight, srcStride, dst, dstStride, dir);
}
...
inline void
fcvElementMultiplyu8u16( const uint8_t * __restrict  src1,  
                         uint32_t                    width,
                         uint32_t                    height,
                         uint32_t                    src1Stride,
                         const uint8_t * __restrict  src2,
                         ...
                         uint32_t                    dstStride )
{
   src1Stride = (src1Stride ==0 ? width : src1Stride);
   src2Stride = (src2Stride ==0 ? width : src2Stride);
   dstStride = (dstStride==0 ?    width*2 : dstStride);
#ifndef FASTCV_DISABLE_API_ENFORCEMENT
    fcvAssert( ((int)src1 & 0xF) == 0 );          // 128-bit alignment
    fcvAssert( ((int)src2 & 0xF) == 0 );          // 128-bit alignment
    fcvAssert( ((int)dst & 0xF) == 0 );           // 128-bit alignment
    fcvAssert( (src1Stride & 0x7) == 0 );         // multiple of 8
    fcvAssert( (src2Stride & 0x7) == 0 );         // multiple of 8
    fcvAssert( (dstStride & 0x7) == 0 );          // multiple of 8
#endif

    (**ppfcvElementMultiplyu8u16) (src1, width, height, src1Stride, src2, src2Stride, dst, dstStride);
}
...
inline void
fcvElementMultiplyf32( const float32_t * __restrict src1,   
                       uint32_t                     width,
                       uint32_t                     height,
                       uint32_t                     src1Stride,
                       const float32_t * __restrict src2, 
                       ...
                       uint32_t                     dstStride )
{
   src1Stride = (src1Stride ==0 ? width*4 : src1Stride);
   src2Stride = (src2Stride ==0 ? width*4 : src2Stride);
   dstStride = (dstStride==0 ?    width*4 : dstStride);
#ifndef FASTCV_DISABLE_API_ENFORCEMENT
    fcvAssert( ((int)src1 & 0xF) == 0 );          // 128-bit alignment
    fcvAssert( ((int)src2 & 0xF) == 0 );          // 128-bit alignment
    fcvAssert( ((int)dst & 0xF) == 0 );           // 128-bit alignment
    ...
    (**ppfcvElementMultiplyf32) (src1, width, height, src1Stride, src2, src2Stride, dst, dstStride);
}
...
fcvMatrixMultiplys8s32( const int8_t * __restrict  src1, 
                        ...
                        uint32_t                   dstStride )
{
   src1Stride = (src1Stride ==0 ? src1Width : src1Stride);
   src2Stride = (src2Stride ==0 ? src2Width : src2Stride);
   dstStride =  (dstStride==0 ?   src2Width*4 : dstStride);
#ifndef FASTCV_DISABLE_API_ENFORCEMENT
    fcvAssert( ((int)src1 & 0xF) == 0 );     // 128-bit alignment
    fcvAssert( ((int)src2 & 0xF) == 0 );     // 128-bit alignment
    fcvAssert( ((int)dst & 0xF) == 0 );      // 128-bit alignment
    ...
    (**ppfcvMatrixMultiplys8s32) (src1, src1Width, src1Height, src1Stride, src2, src2Width, src2Stride, dst, dstStride);
}
...
fcvMatrixMultiplyf32( const float32_t * __restrict   src1, 
                      ...
                      uint32_t                       dstStride )
{
   src1Stride = (src1Stride ==0 ? src1Width*4 : src1Stride);
   src2Stride = (src2Stride ==0 ? src2Width*4 : src2Stride);
   dstStride =  (dstStride==0 ?   src2Width*4 : dstStride);
#ifndef FASTCV_DISABLE_API_ENFORCEMENT
    fcvAssert( ((int)src1 & 0xF) == 0 );     // 128-bit alignment
    fcvAssert( ((int)src2 & 0xF) == 0 );     // 128-bit alignment
    ...
    (**ppfcvMatrixMultiplyf32) (src1, src1Width, src1Height, src1Stride, src2, src2Width, src2Stride, dst, dstStride);
}
...
fcvBlockDotProductu8( const uint8_t * __restrict  src1, 
                      ...
                      uint32_t                    src2Stride )
{
   src1Stride = (src1Stride ==0 ? blockWidth : src1Stride);
   src2Stride = (src2Stride ==0 ? blockWidth : src2Stride);
#ifndef FASTCV_DISABLE_API_ENFORCEMENT
    fcvAssert( (src1Stride & 0x7) == 0 );         // multiple of 8
    fcvAssert( (src2Stride & 0x7) == 0 );         // multiple of 8
#endif

    return (**ppfcvBlockDotProductu8) (src1, blockWidth, blockHeight, src1Stride, src2, src2Stride);
}
...
fcvBlockDotProductf32( const float32_t * __restrict  src1, 
                       ...
                       uint32_t                      src2Stride )
{
   src1Stride = (src1Stride ==0 ? blockWidth*4 : src1Stride);
   src2Stride = (src2Stride ==0 ? blockWidth*4 : src2Stride);
#ifndef FASTCV_DISABLE_API_ENFORCEMENT
    fcvAssert( (src1Stride & 0x7) == 0 );         // multiple of 8
    fcvAssert( (src2Stride & 0x7) == 0 );         // multiple of 8
#endif

    return (**ppfcvBlockDotProductf32) (src1, blockWidth, blockHeight, src1Stride, src2, src2Stride);
}
             ...
             uint32_t                    dstStride )
{
   ...
   dstStride =  (dstStride==0 ?   width*2 : dstStride);
    ...
    (**ppfcvAddu8u16) (src1, width, height, src1Stride, src2, src2Stride, dst, dstStride);
}
...
fcvAdds16( const int16_t * __restrict  src1, 
           ...
           uint32_t                    dstStride )
{
   src1Stride = (src1Stride ==0 ? width*2 : src1Stride);
   src2Stride = (src2Stride ==0 ? width*2 : src2Stride);
   dstStride =  (dstStride==0 ?   width*2 : dstStride);
#ifndef FASTCV_DISABLE_API_ENFORCEMENT
    fcvAssert( ((int)src1 & 0xF) == 0 );          // 128-bit alignment
    fcvAssert( ((int)src2 & 0xF) == 0 );          // 128-bit alignment
    ...
    (**ppfcvAdds16) (src1, width, height, src1Stride, src2, src2Stride, dst, dstStride);
}
           ...
           uint32_t                      dstStride )
{
   ...
   dstStride =  (dstStride==0 ?   width*4 : dstStride);
    ...
    (**ppfcvAddf32) (src1, width, height, src1Stride, src2, src2Stride, dst, dstStride);
}
...
fcvSumOfSquaredDiffsu8( const uint8_t* __restrict                 a,
                        ...
                        float32_t* __restrict                     distances )
{
   return (**ppfcvSumOfSquaredDiffsu8)( a, invLenA, length, bList, invLenB, numB, distances );
}
...
fcvSumOfSquaredDiffsf32( const float32_t* __restrict             a,
                         ...
                         float32_t* __restrict                   distances )
{
   return (**ppfcvSumOfSquaredDiffsf32)( a, invLenA, length, bList, invLenB, numB, distances );
							  ...
							  uint32_t                   dstStride)
{
   ...
   (**ppfcvImageHistogramEqualizeu8)(src,srcWidth,srcHeight,srcStride,dst,dstStride);
                             ...
                             float32_t*__restrict      histogram)
{
   ...
   (**ppfcvImageSpatialHistogramu8)(src,srcWidth,srcHeight,srcStride,numPatterns,grid_x,grid_y,histogram);
}
                   ...
                   uint32_t                   dstStride)
{
   ...
   (**ppfcvScaleDownBLu8)( src, srcWidth, srcHeight, srcStride,
                           dst, dstWidth, dstHeight, dstStride);
}
...
fcvSumOfSquaredDiffss8( const int8_t* __restrict                a,
                        ...
                        float32_t* __restrict                   distances )
{
   return (**ppfcvSumOfSquaredDiffss8)( a, invLenA, length, bList, invLenB, numB, distances );
}
                ...
                fcvLine*                   pLines)
{
   ...
   fcvAssert( (srcStride & 0x7) == 0 );         // multiple of 8
#endif
	return (**ppfcvHoughLineu8)( src,srcWidth,srcHeight,srcStride,threshold,maxLines,pNumLines,pLines );
}
                    ...
                    uint32_t                  dstStride )
{
   ...
   fcvAssert( (dstStride & 0x7) == 0 );    // multiple of 8
#endif
	return (**ppfcvColorCbCrSwapu8)(src,srcWidth,srcHeight,srcStride,dst,dstStride);
}
